    ...
    class ProductsController < ApplicationController
      def index
        if params[:order_id].present?
          product_ids = OrdersProducts.where(order_id: params[:order_id]).pluck(:product_id)
      ...
      def create
        ...
        @order_product = OrdersProducts.new(order_id: @order.id, product_id: order_product_params[:product_id])
